...
# 配置智谱 BigModel
configure_bigmodel() {
    print_info "配置智谱 BigModel..."
    
    echo ""
    echo "请输入您的智谱 BigModel API Key:"
    echo "（API Key 可在 https://open.bigmodel.cn 申请）"
...
EOF
    ...
    print_success "智谱 BigModel 配置完成"
}
...
# 启动 AutoGLM
# 在 Termux 环境中自动使用 Helper APP（如果可用）
cd ~/Open-AutoGLM

# 检查 Helper APP 是否可用
if curl -s --max-time 2 "${AUTOGLM_HELPER_URL:-http://localhost:8080}/status" > /dev/null 2>&1; then
    # Helper APP 可用，使用它
    python main.py --use-helper --helper-url "${AUTOGLM_HELPER_URL:-http://localhost:8080}" "$@"
else
    # Helper APP 不可用，尝试使用 ADB（可能会失败，但让用户知道）
    python main.py "$@"
fi
LAUNCHER_EOF
